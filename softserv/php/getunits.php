...
$frommysql_getunits = array(); //retrieve from assoc array
...
for ($i = 0; $i < count($return_getunits); $i++) {
	$id = $return_getunits[$i]["ID"];
	$name = $return_getunits[$i]["NAME"];
	$start = $return_getunits[$i]["START"];
	$end = $return_getunits[$i]["END"];
	$getunits["$id"] = array("name" => $name,
								   "start" => $start,
								   "end" => $end);
}
